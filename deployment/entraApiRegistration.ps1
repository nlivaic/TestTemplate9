...
Write-Host "##[warning]--- Create Azure App Registration - START ---" -ForegroundColor Yellow
Write-Host "##[section]displayNameApi: $displayNameApi" -ForegroundColor Green
...
Write-Host "##[section]Created API $displayNameApi with appId: $appRegistrationResultAppId" -ForegroundColor Green
Write-Host "##[warning]--- Create Azure App Registration - END ---" -ForegroundColor Yellow

##################################
### Expose API
##################################
Write-Host "##[warning]--- Expose API - START ---" -ForegroundColor Yellow
az ad app update --id $appRegistrationResultAppId --identifier-uris api://$appRegistrationResultAppId
Write-Host "##[section]API $displayNameApi exposed" -ForegroundColor Green
Write-Host "##[warning]--- Expose API - END ---" -ForegroundColor Yellow

##################################
###  Add scopes (oauth2Permissions)
##################################
Write-Host "##[warning]--- Add scopes - START ---" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "##[section]Disabling existing scopes..." -ForegroundColor Green
...
foreach ($scope in $oauth2PermissionScopesApiOld.oauth2PermissionScopes) {
    Write-Host "##[section]`tExisting scope $($scope.value) disabled." -ForegroundColor Green
...
Write-Host "##[section]Existing scopes disabled successfully." -ForegroundColor Green

# 3. Add new scopes from file deployment/oauth2PermissionScopes.json.
Write-Host ""
Write-Host "##[section]Creating scopes..." -ForegroundColor Green
...
if ($? -eq $false) {
    ...
    foreach ($scope in $oauth2PermissionScopesApiOld.oauth2PermissionScopes) {
        Write-Host "##[error]`tExisting scope $($scope.value) re-enabled." -ForegroundColor Red
    ...
    az rest --method PATCH --uri $graphurl --headers $headerJson --body '@deployment/oauth2PermissionScopesOld.json'
    Remove-Item .\deployment\oauth2PermissionScopesOld.json
    Write-Host "##[error]--- Add scopes - END (Error) ---" -ForegroundColor Red
    Return
}
...
foreach ($scope in $oauth2PermissionScopesApi.oauth2PermissionScopes) {
    Write-Host "##[section]`tScope created: $($scope.value)" -ForegroundColor Green
}
Write-Host "##[section]Scopes created successfully." -ForegroundColor Green
Write-Host "##[warning]--- Add scopes - END ---" -ForegroundColor Yellow

Write-Host ""
Write-Host "##[section]Registered App details:" -ForegroundColor Green
Write-Host "##[section]$($appRegistration)" -ForegroundColor Green

##################################
###  Service Principal Lock
##################################
Write-Host "##[warning]--- Service Principal Lock - START ---" -ForegroundColor Yellow
az rest --method PATCH --uri $graphurl --headers $headerJson --body '@deployment/servicePrincipalLockConfiguration.json'
Write-Host "##[section]Service Principal Locked." -ForegroundColor Green
Write-Host "##[warning]--- Service Principal Lock - END ---" -ForegroundColor Yellow

##################################
###  Create a Service Principal for the API App Registration
##################################
Write-Host "##[warning]--- Create a ServicePrincipal - START ---" -ForegroundColor Yellow

$createdSp = az ad sp show --id $appRegistrationResultAppId
if ($? -eq $false) {
    $createdSp = az ad sp create --id $appRegistrationResultAppId
    Write-Host "##[section]Created Service Principal for API App registration" -ForegroundColor Green
} else {
    Write-Host "##[section]Service Principal already exists, skipped creation." -ForegroundColor Green
}
Write-Host "##[section]Service principal details:" -ForegroundColor Green
Write-Host "##[section]$($createdSp)"" -ForegroundColor Green
Write-Host "##[warning]--- Create a ServicePrincipal - END ---" -ForegroundColor Yellow
